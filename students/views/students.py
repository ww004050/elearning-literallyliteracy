...
class QuizListView(ListView):
    ...
    def get_queryset(self):
        try:
            ...
            queryset = Quiz.objects.filter(tags__in=student_interests).exclude(pk__in=taken_quizzes).annotate(
                question_count=Count('questions')).filter(question_count__gt=0)
            return queryset
        except ObjectDoesNotExist:
            return self.request.user
...
def student_recommendation_list(request):
    ...
    others_users_reviews = Review.objects.filter(user_name__in=others_members_usernames).exclude(
        course__id__in=user_review_course_ids)
    ...
    return render(request, 'students/reviews/student_recommendation_list.html',
                  {'student': request.user.username, 'course_list': course_list})
