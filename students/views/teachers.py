...
class TeacherQuizListView(ListView):
    ...
    def get_queryset(self):
        queryset = self.request.user.quizzes.select_related('tags').annotate(
            questions_count=Count('questions', distinct=True)).annotate(
            taken_count=Count('taken_quizzes', distinct=True))
        return queryset
...
class QuizCreateView(CreateView):
    model = Quiz
    # fields = ('name', 'tags',)
    form_class = modelform_factory(Quiz, exclude=['owner'],
