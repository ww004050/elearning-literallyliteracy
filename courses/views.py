...
def list_videos(request):
    ...
    return render(request, 'videos/list.html',
                  {'videos': videos, 'q': q, 'results': results, 'subjects': subjects, 'max_lengths': max_lengths})
...
class CourseListView(TemplateResponseMixin, View):
    ...
    def get(self, request, subject=None):
        ...
        all_courses = Course.objects.annotate(total_modules=Count('modules', distinct=True)).annotate(
            total_reviews=Count('reviews', distinct=True)).annotate(
            average_rating=Avg(F('reviews__rating'), distinct=True))
        ...
        Form = modelform_factory(model, exclude=['owner', 'order', 'created', 'updated'],
                                 widgets={'title': forms.TextInput(attrs={'class': 'form-control'}),
                                          'content': forms.Textarea(
                                              attrs={'class': 'form-control', 'cols': 40, 'rows': 8}),
